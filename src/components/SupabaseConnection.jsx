		...
		<div className="flex flex-col lg:flex-row items-center justify-center lg:items-start gap-5 select-none">
			...
			<div className="flex flex-col items-center justify-center gap-2">
				<div className="flex flex-row items-center justify-center gap-2 text-xs text-zinc-700 border border-zinc-700 bg-zinc-200 rounded-full px-3 py-1.5">
					...
					<p>Supabase Publishable Key: <span className={`font-semibold ${hasKey ? `text-green-600` : `text-red-600`}`}>{hasKey ? "Set" : "Not Set"}</span></p>
				</div>
				{!hasKey && (
					<p className="text-xs text-red-600">Please set the <code className="font-mono bg-zinc-200 px-1 py-0.5 text-zinc-800 font-light">VITE_SUPABASE_PUBLISHABLE_DEFAULT_KEY</code> environment variable in your .env file.</p>
				)}
			</div>
		</div>
	);
